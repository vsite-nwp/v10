...
{
	int height = pDC->GetTextExtent("X").cy;
	...
	pDC->TextOut(userID, y, "ID");
	pDC->TextOut(userName, y, "name");
	pDC->TextOut(isManager, y, "manager");
	y += height;
	...
	while (!rs.IsEOF()) {
		CString _id;
		_id.Format(_T("%d"), rs.m_id);

		pDC->TextOut(userID, y, _id);
		pDC->TextOut(userName, y, rs.m_name);
		if (rs.m_manager)
			pDC->TextOut(isManager, y, "x");
		y += height;
