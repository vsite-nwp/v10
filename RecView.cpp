...
void RecView::OnPrint(CDC* pDC, CPrintInfo* pInfo)
{
	...
	CSize const redak = pDC->GetTextExtent("ABC");
	int x = pDC->GetDeviceCaps(HORZRES);
	int y = pDC->GetDeviceCaps(VERTRES);
	x = x / 9;
	y = y / redak.cy;
	pDC->TextOutA(x, y, "  id");
	pDC->TextOutA(x*4, y, "name");
	pDC->TextOutA(x*7, y, "manager");
	pDC->MoveTo(x, y + redak.cy);
	pDC->LineTo(x * 8, y + redak.cy);
	...
	while (!rs.IsEOF()) {
		...
		y += redak.cy;//enter
		CString id;
		id.Format("%d", rs.m_id); // tip long u string
		pDC->TextOut(x, y, id);
