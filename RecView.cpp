...
{
	const int widht = pDC->GetDeviceCaps(HORZRES);
	const int height = pDC->GetDeviceCaps(VERTRES);

	const CSize TExtend = pDC->GetTextExtent("AA");

	const int x = widht / 20;
	int y = TExtend.cy * 10;

	pDC->TextOutA(x, y, "id");
	pDC->TextOutA(x * 3, y, "name");
	pDC->TextOutA(x * 5, y, "manager");

	pDC->MoveTo(x, y += TExtend.cy);
	pDC->LineTo(x * 10, y);
